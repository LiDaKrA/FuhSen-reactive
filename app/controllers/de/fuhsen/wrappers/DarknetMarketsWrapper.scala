...
/**
  ...
  */
class DarknetMarketsWrapper extends RestApiWrapperTrait with SilkTransformableTrait {
  ...
  override def apiUrl: String = ConfigFactory.load.getString("crawler.darknetmarkets.rest.api.search")

  /**
    * Returns the globally unique URI String of the source that is wrapped. This is used to track provenance.
    */
  override def sourceLocalName: String = "darknetmarkets"

  /** SILK Transformation Trait **/
  override def silkTransformationRequestTasks = Seq(
    SilkTransformationTask(
      transformationTaskId = ConfigFactory.load.getString("silk.transformation.task.darknetmarkets.product"),
      createSilkTransformationRequestBody(
        basePath = "hits/hits",
        uriPattern = "http://vocab.cs.uni-bonn.de/fuhsen/search/entity/darknetmarkets/{id}"
      )
    )
  )
